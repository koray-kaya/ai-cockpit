...
export interface IStorage {
  ...
  // Bootcamp
  getBootcampDays(): BootcampDay[];
  updateBootcampDay(id: number, data: Partial<InsertBootcampDay>): BootcampDay | undefined;
  // Skills
  getSkills(): Skill[];
  updateSkill(id: number, data: Partial<InsertSkill>): Skill | undefined;
  // Journal
  getJournalEntries(): JournalEntry[];
  createJournalEntry(data: InsertJournalEntry): JournalEntry;
  updateJournalEntry(id: number, data: Partial<InsertJournalEntry>): JournalEntry | undefined;
  deleteJournalEntry(id: number): void;
  // Visibility
  getVisibilityLogs(): VisibilityLog[];
  createVisibilityLog(data: InsertVisibilityLog): VisibilityLog;
  updateVisibilityLog(id: number, data: Partial<InsertVisibilityLog>): VisibilityLog | undefined;
  // Resources
...
export class DatabaseStorage implements IStorage {
  ...
  // Bootcamp
  getBootcampDays(): BootcampDay[] {
    return db.select().from(bootcampDays).all();
  }
  updateBootcampDay(id: number, data: Partial<InsertBootcampDay>): BootcampDay | undefined {
    return db.update(bootcampDays).set(data).where(eq(bootcampDays.id, id)).returning().get();
  }
  // Skills
  getSkills(): Skill[] {
  ...
  // Visibility
  getVisibilityLogs(): VisibilityLog[] {
    return db.select().from(visibilityLogs).orderBy(desc(visibilityLogs.weekOf)).all();
  }
  createVisibilityLog(data: InsertVisibilityLog): VisibilityLog {
    return db.insert(visibilityLogs).values(data).returning().get()!;
  }
  updateVisibilityLog(id: number, data: Partial<InsertVisibilityLog>): VisibilityLog | undefined {
    return db.update(visibilityLogs).set(data).where(eq(visibilityLogs.id, id)).returning().get();
  }
  // Resources
  getResources(): Resource[] {
...
function seedDatabase() {
  const existingPhases = db.select().from(phases).all();
  if (existingPhases.length > 0) return; // Already seeded
  ...
  for (const d of bootcampData) {
    db.insert(bootcampDays).values(d).run();
  }
